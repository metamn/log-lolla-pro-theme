...
<h3 class="comment-list-title list-title">
	...
	if ( 1 === $number_of_comments ) {
		...
		$text = esc_html_x( 'One update', 'one comment', 'log-lolla-pro-theme' );
	} else {
		/* translators: The Comment section title for multiple comments. */
		$text = $number_of_comments . esc_html_x( ' updates', ' comments', 'log-lolla-pro-theme' );
	}

	$arrows  = log_lolla_pro_theme_get_arrow_html( 'bottom' );
	$arrows .= log_lolla_pro_theme_get_arrow_html( 'bottom' );
	$arrows .= log_lolla_pro_theme_get_arrow_html( 'bottom' );
